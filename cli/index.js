...
exports.commands = {}; //object that defines information about a plugin
...
exports.use(plugins);
addCustomCommands(); //#endregion PLUGIN_ENTRY_POINT
//creates a new project from a template

exports.newTemplate = function (element) {
  mem.newVar(element[1], 'name'); //we return the exit status

  return template.loadTemplates(element);
};

tp.error(token => {
  logger(error('error: unrecognized token ', 'parser_unrecognized_token') + chalk.whiteBright("'" + token + "'."), 'error');
});
tp.add('verbose', () => {
...
}, 'Suppresses info and success logs.'); //add the new command, with template and name
...
tp.add('plugin <option> <pluginName>', element => {
  if (element[0] === 'info') {
    ...
    if (exports.pluginInfo[element[1]]) {
      ...
      //log info about a plugin
      console.log(chalk.cyan('info about plugin ') + chalk.whiteBright(element[1] + ':'));
      logger('version: ' + chalk.whiteBright(template.pluginInfo[element[1]]['version'] || INFO_NOT_GIVEN), 'info'); //console.log(process.env.TGENPATH);

      logger('author: ' + chalk.whiteBright(template.pluginInfo[element[1]]['author'] || INFO_NOT_GIVEN), 'info');
      logger('repo: ' + chalk.whiteBright(template.pluginInfo[element[1]]['repo'] || INFO_NOT_GIVEN), 'info');
      logger('extends: ' + chalk.whiteBright(template.pluginInfo[element[1]]['extends'] || INFO_NOT_GIVEN), 'info');
      logger('description: ' + chalk.whiteBright(template.pluginInfo[element[1]]['description'] || INFO_NOT_GIVEN), 'info');
    } else {
      if (element[1]) {
  ...
  if (element[0] === 'ignore') {
    //ignore a plugin
    console.log(chalk.cyanBright('ignoring plugin: ' + element[1])); //pushes to tgenSetting the plugin to ignore

    mem.tgenSettings['plugins']['ignore'].push(element[1]); //writes the changes to tgen.yaml
    // $FlowFixMe
...
if (typeof global.it !== 'function') {
  ...
  const exitStatus = tp.parse(); //separator
